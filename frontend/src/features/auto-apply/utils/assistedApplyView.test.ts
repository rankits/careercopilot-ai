...
import type { AssistedApplyViewState } from './assistedApplyView';
import { toAssistedApplyView, labelForViewState, tooltipForViewState } from './assistedApplyView';
...
describe('assistedApplyView', () => {
  const exhaustiveMatrix: Record<JobApplicationDto['status'], AssistedApplyViewState> = {
    ...
    WITHDRAWN: 'ABANDONED',
    COULD_NOT_APPLY: 'ABANDONED',
    JOB_CLOSED: 'ABANDONED',
  ...
  it('falls back to LEGACY_ATTENTION for unknown status and logs error', () => {
    ...
    expect(toAssistedApplyView('SOME_FUTURE_STATUS' as JobApplicationDto['status'])).toBe(
      'LEGACY_ATTENTION',
    );
  ...
  it('provides the correct tooltip only for LEGACY_ATTENTION', () => {
    expect(tooltipForViewState('LEGACY_ATTENTION')).toBe(
      'This application was left in an older processing state. You can abandon it and start Assisted Apply again.',
    );
