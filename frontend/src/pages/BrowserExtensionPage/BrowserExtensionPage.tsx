...
import { extensionService, type ExtensionDevice, type PairingSession } from '@/features/extension';
import {
...
export function BrowserExtensionPage() {
  ...
  const startPairing = async () => {
    ...
    try {
      ...
      requestAnimationFrame(() =>
        document
          .getElementById('connect-browser-extension')
          ?.scrollIntoView({ behavior: 'smooth' }),
      );
    } catch (requestError) {
      setError(
        requestError instanceof Error ? requestError.message : 'Unable to create pairing code.',
      );
    } finally {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Box sx={{ flex: 1 }}>
          <Typography fontWeight={700}>
            Use the Career Copilot browser extension to autofill applications
          </Typography>
          <Typography color="text.secondary" variant="body2">
        ...
        <>
          <Paper sx={{ mb: 2.5, p: { xs: 3, sm: 5 }, textAlign: 'center' }} variant="outlined">
            ...
            <Typography color="text.secondary" sx={{ mb: 2.5 }} variant="body2">
              Connect your first extension to get started.
            </Typography>
          ...
          <Paper sx={{ mb: 2.5, p: { xs: 2, sm: 3 } }} variant="outlined">
            <Typography fontWeight={700} variant="h6">
              Install Career Copilot Extension
            </Typography>
              ...
              {browserOptions.map((browser) => (
                ...
                >
                  <Box component="span" sx={{ alignItems: 'center', display: 'flex', gap: 1 }}>
                    <Box component="span" sx={{ color: browser.color, fontWeight: 800 }}>
                      {browser.mark}
                    </Box>
                    {browser.name}
                  </Box>
          ...
          <Paper sx={{ mb: 2.5, p: { xs: 2, sm: 3 } }} variant="outlined">
            ...
            >
              {[
                ...
                <Box
                  key={title}
                  sx={{ borderRight: { sm: '1px solid' }, borderColor: 'divider', pr: 2 }}
                >
                  ...
                  >
                    ...
                    <Box sx={{ flex: 1, minWidth: 0 }}>
                      <Box sx={{ alignItems: 'center', display: 'flex', gap: 1 }}>
                        <Typography fontWeight={700}>
                          {browser.name} on {platformName(device.userAgent)}
                        </Typography>
                        <Chip color="success" label="Active" size="small" />
                      </Box>
                      <Typography color="text.secondary" noWrap variant="caption">
                        {device.userAgent ?? 'Browser extension'}
                      </Typography>
      ...
      {pairing ? (
        <Paper
          id="connect-browser-extension"
          sx={{ mb: 2.5, p: { xs: 2, sm: 3 } }}
          variant="outlined"
        >
            ...
            >
              ...
              <Box>
                ...
                <Typography color="text.secondary" variant="caption">
                  We use encrypted communication to keep your data safe.
                </Typography>
              </Box>
            </Box>
          </Box>
          ...
          <Button onClick={() => setPairing(null)} sx={{ mt: 2 }}>
            Done
          </Button>
        </Paper>
      ) : null}
      ...
      <Alert icon={<HelpOutlineIcon />} severity="info">
        <Typography variant="body2">
          <strong>Tip:</strong> Install the Career Copilot extension from an official browser store
          to ensure security and receive updates.
        </Typography>
      </Alert>
    </Box>
  );
