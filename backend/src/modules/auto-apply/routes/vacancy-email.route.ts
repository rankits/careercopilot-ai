...
import { discoverVacancyEmailController } from '@/modules/auto-apply/controllers/vacancy-email.controller.js';
...
router.get(
  '/:jobId',
  ...requireUser,
  requirePermission(AUTO_APPLY_PERMISSIONS.VACANCY_EMAIL_READ_OWN),
  validateResource(
    z.object({ params: z.object({ jobId: z.string().uuid('Invalid job ID format') }) }),
  ),
  discoverVacancyEmailController,
);
