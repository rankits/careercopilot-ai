...
export const UpdateApprovedResumeVersionSchema = z
  .object({
    label: z.string().trim().min(1).max(120).optional(),
    category: z.string().trim().min(1).max(60).optional(),
    tags: z.array(z.string().trim().min(1).max(40)).max(12).optional(),
    isActive: z.boolean().optional(),
    /** AA-026: alias for setting the single default approved resume (`isActive` in storage). */
    isDefault: z.boolean().optional(),
  })
  .transform(({ isDefault, isActive, ...rest }) => ({
    ...rest,
    isActive: isDefault !== undefined ? isDefault : isActive,
  }));
