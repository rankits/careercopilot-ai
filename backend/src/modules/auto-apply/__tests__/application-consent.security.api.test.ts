...
describe('auto-apply consent IDOR / authz', () => {
  it.each([
    ...
    const res = method === 'POST' ? await req.send({ consentType: 'EMAIL_SUBMISSION' }) : await req;
  ...
  it('grants consent scoped to the caller principal', async () => {
    ...
    const res = await request(app)
      ...
      .send({ consentType: 'EMAIL_SUBMISSION' });

    expect(res.status).toBe(201);
    expect(spy).toHaveBeenCalledWith(String(user.id), 'EMAIL_SUBMISSION');
  });
});
