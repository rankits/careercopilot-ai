...
const API = '/api/v1/auto-apply/channel';
...
describe('auto-apply channel detection IDOR / authz', () => {
  ...
  it('rejects a non-uuid jobId with 400', async () => {
    const user = await seedVerifiedUser({ email: 'channel-badid@example.com' });
  ...
  it('detects the channel for an authenticated user', async () => {
    const user = await seedVerifiedUser({ email: 'channel-owner@example.com' });
    const token = accessTokenForUser(user);

    const spy = vi
      .spyOn(channelDetectionService, 'detectChannel')
      .mockResolvedValue({ channel: 'EXTERNAL_MANUAL', reason: 'ok' });

    const res = await request(app).get(`${API}/${JOB_ID}`).set(authHeader(token));

    expect(res.status).toBe(200);
    expect(spy).toHaveBeenCalledWith(JOB_ID);
    expect(res.body.data.channel).toBe('EXTERNAL_MANUAL');
  });
});
