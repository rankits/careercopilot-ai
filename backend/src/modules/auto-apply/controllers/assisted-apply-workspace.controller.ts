...
export const getAssistedApplyWorkspaceController = async (
  ...
  try {
    ...
    const workspace = await assistedApplyWorkspaceService.getWorkspace(userId, id);
...
export const updateWorkspaceProgressStepController = async (
  ...
  try {
    const userId = requireUserPrincipalId(req);
    const id = getParam(req.params.id, 'id');
    const result = await assistedApplyWorkspaceService.updateProgressStep(
      userId,
      id,
      req.body.progressStep,
    );
