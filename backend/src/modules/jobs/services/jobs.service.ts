...
export class JobsService implements IJobContract {
  ...
  async searchJobs(
    ...
    const aggregationResult = await this.aggregationService.aggregateJobs(filters);
  ...
  async getJobById(jobId: string): Promise<NormalizedJob | null> {
    ...
    for (const provider of allProviders) {
      try {
        ...
        const found = jobs.find((j) => j.id === jobId || j.providerJobId === jobId);
        if (found) {
          jobsLogger.info(
            {
              jobId,
  ...
  async triggerBulkIngestion(options?: BulkIngestionOptions): Promise<BulkIngestionSummary> {
