...
export const adminSwagger = {
    ...
    `${BASE_URL}/auth/login`,
    {
      ...
      responses: {
        200: {
          ...
          schema: successSchema('Login successful', {
  ...
  ...createApiPost(
    `${BASE_URL}/auth/refresh-token`,
    {
      ...
      description:
        'Presenting an already-rotated (reused) token revokes every session for that admin as a precaution.',
      body: { required: ['refreshToken'], properties: { refreshToken: { type: 'string' } } },
      responses: {
        200: {
          description: 'Session refreshed',
          schema: successSchema('Session refreshed', authTokensSchema),
        },
        401: {
    ...
    `${BASE_URL}/auth/me`,
    {
      ...
      responses: {
        200: {
          ...
          schema: successSchema('Current session', safeAdminSchema),
        },
        ...commonAdminResponses,
      },
    },
