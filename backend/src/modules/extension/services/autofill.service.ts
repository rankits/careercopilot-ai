...
export class AutofillService {
  async generateAnswers(
    ...
    if (!apiKey) {
      throw new AppError(
        'AI API key not configured (set OPENROUTER_API_KEY or OPENAI_API_KEY)',
        500,
        'AI_UNAVAILABLE',
      );
    }
    ...
    const profileText = profile
      ? JSON.stringify(profile.preferences) + ' ' + JSON.stringify(profile.links)
...
${JSON.stringify(fields, null, 2)}`;
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error(
          `[AutofillService] ${providerName} API Error: ${response.status} - ${errorText}`,
        );

        if (response.status === 402) {
          throw new AppError(
            'AI provider has insufficient credits to complete this request.',
            502,
            'AI_ERROR',
          );
        }
        if (response.status === 401 || response.status === 403) {
          throw new AppError(
            'AI provider configuration is invalid or unauthorized.',
            502,
            'AI_ERROR',
          );
        }
