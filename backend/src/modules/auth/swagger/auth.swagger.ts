...
const rememberMeField = {
  type: "boolean",
  default: false,
  description: "If true, the session survives at the long-lived JWT_REFRESH_EXPIRES_IN TTL instead of the short one.",
};
...
const genericOtpSentResponse = {
  description: "Generic confirmation (identical whether or not an account exists, to avoid account enumeration)",
...
export const authSwagger = {
    ...
    `${BASE_URL}/register`,
    {
      ...
      responses: {
        201: {
          ...
          schema: successSchema("Registration initiated. Please verify the code sent to your email.", {
            email: emailField,
          }),
        },
        400: { description: "Validation error", schema: errorSchema("Payload is incorrect or missing fields.") },
        409: {
  ...
  ...createApiPost(
    `${BASE_URL}/otp/resend`,
    {
      ...
      description: "Re-issues a code for the given purpose (registration / login / password_reset).",
      body: {
  ...
  ...createApiPost(
    `${BASE_URL}/otp/verify-registration`,
    {
      ...
      description: "Completes registration: activates the account and issues a session (refresh token via httpOnly cookie).",
      body: {
        required: ["email", "code"],
      ...
      responses: {
        200: {
          ...
          headers: {
            "Set-Cookie": { description: "httpOnly `refreshToken` cookie", schema: { type: "string" } },
          },
        },
  ...
  ...createApiPost(
    `${BASE_URL}/login/otp/request`,
    {
      ...
      description: "Emails a login code if, and only if, the account exists, is verified, and is active - the response is identical either way.",
  ...
  ...createApiPost(
    `${BASE_URL}/reset-password`,
    {
      ...
      description: "On success, every existing session for the account is revoked (a fresh login is required).",
      body: {
  ...
  ...createApiPost(
    `${BASE_URL}/refresh-token`,
    {
      ...
      body: { properties: { refreshToken: { type: "string", description: "Fallback only - cookie is preferred." } } },
      responses: {
        200: {
          description: "Session refreshed",
          schema: successSchema("Session refreshed", undefined, accessTokenFields),
          headers: {
            "Set-Cookie": { description: "Rotated httpOnly `refreshToken` cookie", schema: { type: "string" } },
          },
        },
        401: {
  ...
  ...createApiPost(
    `${BASE_URL}/logout-all`,
    {
      ...
      description: "Revokes all of the caller's sessions and bumps their token version, invalidating every currently-live access token too.",
      responses: {
  ...
  ...createApiPost(
    `${BASE_URL}/change-password`,
    {
      ...
      description: "On success, every session (including the one used to make this request) is revoked.",
      body: {
  ...
  ...createApiGet(
    `${BASE_URL}/me`,
    {
      ...
      responses: {
        200: { description: "Current user profile", schema: successSchema("Current session", safeUserSchema) },
        ...commonSecureResponses,
      },
    },
    true,
