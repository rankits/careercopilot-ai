...
describe('recommendationRateLimiter', () => {
  beforeEach(() => {
    vi.restoreAllMocks();
  });

  it('keys authenticated USER requests by principal id', async () => {
    ...
    expect(increment).toHaveBeenCalledWith(
      'careercopilot:ratelimit:recommendation:42',
      expect.any(Number),
    );
  ...
  it('returns 429 when the threshold is exceeded', async () => {
    ...
    recommendationRateLimiter({ ip: '1.2.3.4' } as Request, {} as Response, next);
    await vi.waitFor(() => expect(next).toHaveBeenCalled());
