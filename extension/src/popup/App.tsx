...
const App: React.FC = () => {
    ...
    try {
      ...
      try {
        ...
        if (err.message.includes('Receiving end does not exist')) {
          throw new Error("Content script not found. Please refresh the page and try again (or ensure you aren't on a restricted page).");
        }
        throw err;
      }
  ...
  if (isAuthenticated) {
    return (
      <div style={{ padding: '0', fontFamily: 'system-ui, -apple-system, sans-serif', width: '400px', background: 'white', borderRadius: '12px', boxSizing: 'border-box', color: '#111827', display: 'flex', flexDirection: 'column' }}>
        
        {/* Header */}
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 20px', borderBottom: '1px solid #f3f4f6' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <img src="/assets/logo/career-copilot-full-logo.svg" alt="Career Copilot" style={{ height: '24px' }} onError={(e) => (e.currentTarget.style.display = 'none')} />
          </div>
        ...
        <div style={{ padding: '20px' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              ...
              <div>
                <div style={{ color: '#10b981', fontWeight: 600, fontSize: '15px' }}>Connected to web app</div>
                <div style={{ color: '#6b7280', fontSize: '13px' }}>Your account is synced and ready.</div>
              </div>
            </div>
            <button style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '6px 12px', border: '1px solid #e5e7eb', borderRadius: '20px', background: 'white', color: '#3b82f6', fontSize: '13px', fontWeight: 500, cursor: 'pointer' }}>
          ...
          <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '12px', padding: '20px', marginBottom: '24px' }}>
            <div style={{ display: 'flex', gap: '16px', marginBottom: '20px' }}>
              <div style={{ width: '56px', height: '56px', background: '#e0e7ff', borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              ...
              <div>
                <h3 style={{ margin: '0 0 6px 0', fontSize: '18px', fontWeight: 700, color: '#0f172a' }}>Auto-Fill Form</h3>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              <button 
                ...
                disabled={isFilling}
                style={{ 
                  width: '100%', padding: '12px', background: isFilling ? '#9ca3af' : '#16a34a', color: 'white', 
                  border: 'none', borderRadius: '8px', cursor: isFilling ? 'wait' : 'pointer', fontWeight: 600, fontSize: '15px',
                  display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', transition: 'background 0.2s'
                }}
              >
                <div style={{ background: 'white', borderRadius: '50%', width: '18px', height: '18px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#16a34a' }}>
              ...
              <button 
                ...
                disabled={isFilling}
                style={{ 
                  width: '100%', padding: '12px', background: isFilling ? '#9ca3af' : '#2563eb', color: 'white', 
                  border: 'none', borderRadius: '8px', cursor: isFilling ? 'wait' : 'pointer', fontWeight: 600, fontSize: '15px',
                  display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', transition: 'background 0.2s'
                }}
              >
            ...
            {fillResult && (
              <div style={{ marginTop: '12px', fontSize: '13px', textAlign: 'center', fontWeight: 500, color: fillResult.startsWith('Error') ? '#ef4444' : '#16a34a' }}>
                {fillResult}
              </div>
            )}
          </div>
          ...
          <button 
            ...
            style={{ 
              ...
              border: '1px solid #fca5a5', borderRadius: '8px', cursor: 'pointer', fontWeight: 500, fontSize: '14px',
        ...
        {/* Footer */}
        <div style={{ background: '#f8fafc', padding: '12px 20px', borderTop: '1px solid #f1f5f9', borderRadius: '0 0 12px 12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '13px', color: '#64748b', fontWeight: 500 }}>
            <img src="/assets/logo/career-copilot-full-logo.svg" alt="" style={{ height: '16px' }} onError={(e) => (e.currentTarget.style.display = 'none')} />
            <span>•</span>
            <span>v1.2.0</span>
          </div>
  ...
  return (
    <div style={{ padding: '24px', fontFamily: 'system-ui, -apple-system, sans-serif', width: '420px', background: 'white', borderRadius: '16px', boxSizing: 'border-box', color: '#111827' }}>
      {/* Header */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <img src="/assets/logo/career-copilot-full-logo.svg" alt="Career Copilot" style={{ height: '28px' }} onError={(e) => (e.currentTarget.style.display = 'none')} />
        <button 
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center' }}>
        {/* Big Icon */}
        <div style={{ background: '#e0e7ff', borderRadius: '16px', width: '64px', height: '64px', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '16px' }}>
          <svg width="32" height="32" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" stroke="#1d4ed8" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
        </div>

        <h2 style={{ margin: '0 0 12px 0', fontSize: '24px', fontWeight: 700, color: '#111827' }}>Connect Extension</h2>
        <p style={{ margin: '0 0 32px 0', fontSize: '15px', color: '#4b5563', lineHeight: '1.5', maxWidth: '300px' }}>
          Enter the 6-digit pairing code from the web app settings to connect this browser.
        </p>

        <form onSubmit={handlePair} style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: '24px' }}>
          {/* Input Grid */}
        ...
        {/* Divider Link */}
        <div style={{ display: 'flex', alignItems: 'center', width: '100%', marginTop: '32px', gap: '16px' }}>
          <div style={{ flex: 1, height: '1px', background: '#e5e7eb' }}></div>
          <a href="#" style={{ display: 'flex', alignItems: 'center', gap: '6px', color: '#2563eb', textDecoration: 'none', fontSize: '14px', fontWeight: 500 }}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M12 22C17.5228 22 22 17.5228 22 12C22 6.47715 17.5228 2 12 2C6.47715 2 2 6.47715 2 12C2 17.5228 6.47715 22 12 22Z" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
              <path d="M9.09009 9.00008C9.32519 8.33175 9.78924 7.76819 10.4001 7.40921C11.0111 7.05022 11.729 6.91899 12.4273 7.03879C13.1255 7.15858 13.7589 7.52161 14.2152 8.06361C14.6714 8.60561 14.9211 9.2916 14.9201 10.0001C14.9201 12.0001 11.9201 13.0001 11.9201 13.0001" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
      ...
      {/* Footer */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '40px', paddingTop: '16px', background: '#f9fafb', margin: '40px -24px -24px -24px', padding: '16px 24px', borderRadius: '0 0 16px 16px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px', color: '#6b7280', fontSize: '12px' }}>
          ...
          <img src="/assets/logo/career-copilot-penguin.svg" alt="" style={{ width: '20px', height: '20px' }} onError={(e) => (e.currentTarget.style.display = 'none')} />
        </div>
      </div>
    </div>
  );
