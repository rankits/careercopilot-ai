...
export class ViewportConstraintManager {
  static getSafeBounds(panelWidth: number, panelHeight: number, safeInset: number = 12): ViewportBounds {
  ...
  static clampPosition(x: number, y: number, panelWidth: number, panelHeight: number, safeInset: number = 12): { x: number, y: number } {
    ...
    const clampedY = Math.max(bounds.top, Math.min(y, maxY > bounds.top ? maxY : bounds.bottom - 60));
