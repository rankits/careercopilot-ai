  ...
  const requestDraft = async (f: FieldInfo) => {
    ...
    try {
      ...
      if (response.success && response.data?.draft) {
        saveState({
          draftValues: { ...panelState.draftValues, [f.identifier]: response.data.draft },
        });
      ...
      <PanelHeader
        onDragStart={(e) =>
          dragController.startDrag(
            ...
            panelRef.current?.getBoundingClientRect().top || 0,
          )
        }
        onClose={onCancel}
      ...
      <div style={{ padding: '0 20px', overflowY: 'auto', flex: 1 }}>
        <ReviewFieldList
          ...
          onDraftChange={(id, val) =>
            saveState({ draftValues: { ...panelState.draftValues, [id]: val } })
          }
        />
      </div>
      ...
      <PanelFooter
        ...
        onResizeStart={(e) =>
          resizeController.startResize(
            e,
            panelRef,
            panelRef.current?.getBoundingClientRect().height || 500,
          )
        }
      />
    </div>
